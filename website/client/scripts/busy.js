// BUSY.JS
// used for waiting animation in camera
/*
Load Indicator script

 Add:
	OBJECT = getBusyOverlay(parent[,overlay[,busy]]);

	parent		== element to add the overlay (e.g. $(id) or 'viewport')
...
function onIEVPResize(event) {
	if(!event) {event=window.event;} var vp=$('viewport'); if(vp) {
...
function getBusyOverlay(parent,overlay,busy) {
	if((typeof(parent)==='object' || parent=='viewport') && document.getElementsByTagName) {
		...
		if(parent=='viewport') {viewport=$('viewport');
