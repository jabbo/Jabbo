...
$dbdates = array();
$res = mysql_query("SELECT day, minutes FROM graph;");
if (mysql_num_rows($res) >= 1) {
	while ($row = mysql_fetch_array($res)) {
		$dbdates[$row['day']] = $row['minutes']/60;
	}
}
...
for ($i = 0; $i < count($aDays); $i++) {
 ...
 $MyData = new pData();
$MyData->addPoints($data,"Hours");
$MyData->setSerieWeight("Hours",2);
$MyData->setSerieOnAxis("Hours",0);
...
$MyData->setAxisUnit(0,"");
 ...
 $myPicture->drawText(150,35,"Hours played",array("FontSize"=>20,"Align"=>TEXT_ALIGN_BOTTOMMIDDLE));
